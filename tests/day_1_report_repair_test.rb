...
class ReportRepairTest < MiniTest::Test
  ...
  def setup
    ...
    @report_repair = ReportRepair.new
    @sum = 2020
  end

  def test_it_can_get_expense_report_repair
    assert_equal 514579, @report_repair.expense_report_repair_two_values(@expense_report_1, @sum)
    assert_equal 651651, @report_repair.expense_report_repair_two_values(@expense_report_2, @sum)
  end

  def test_it_can_get_expense_report_repair_three
    assert_equal 241861950, @report_repair.expense_report_repair_three_values(@expense_report_1, @sum)
    assert_equal 214486272, @report_repair.expense_report_repair_three_values(@expense_report_2, @sum)
  end
end
